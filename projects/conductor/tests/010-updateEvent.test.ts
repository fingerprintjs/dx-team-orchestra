...
function checkUpdatedEvent(updatedEvent: EventsGetResponse) {
  ...
  propertiesToValidate.forEach((property) => {
    expect(updatedResponseBody[property]).toStrictEqual(testData.updateEvent[property])
  })
}
...
test.describe('UpdateEvents Suite', () => {
  ...
  test('for valid complex tag only', async ({ sdkApi, fingerprintApi, identify }) => {
    ...
    expect(identificationData.tag).toStrictEqual(testData.updateEventComplexTag.tag)
    expect(identificationData.linkedId).toStrictEqual(initialResponseBody.linkedId)
  ...
  test('for linkedId only', async ({ sdkApi, fingerprintApi, identify }) => {
    ...
    const updatedResponseBody = updatedEvent.products.identification.data
    expect(updatedResponseBody.linkedId).toStrictEqual(testData.updateEvent.linkedId)
    expect(updatedResponseBody.suspect).toStrictEqual(initialResponseBody.suspect)
